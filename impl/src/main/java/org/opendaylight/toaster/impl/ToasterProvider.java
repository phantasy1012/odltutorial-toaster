...
public class ToasterProvider implements BindingAwareProvider, ToasterService, AutoCloseable {
	...
	private static final DisplayString TOASTER_MANUFACTURER = new DisplayString(
			"Opendaylight");
	private static final DisplayString TOASTER_MODEL_NUMBER = new DisplayString(
	...
	/**
	 * Restconf RPC call implemented from the ToasterService interface. Cancels
	 * the current toast. Implementation to be filled in a later chapter. in
	 * postman, http://localhost:8181/restconf/operations/toaster:cancel-toast
	 */
	@Override
	...
	/**
	 * RestConf RPC call implemented from the ToasterService interface. Attempts
	 * to make toast. Implementation to be filled in a later chapter. in
	 * postman, http://localhost:8181/restconf/operations/toaster:make-toast {
	 * "input" : { "toaster:toasterDoneness" : "10",
	 ...
	 * data store. Note - we are simulating a device whose manufacture and model
	 * are fixed (embedded) into the hardware. / This is why the manufacture and
	 * model number are hardcoded
	 */
	private void initToasterOperational() {
		...
		LOG.info("initToasterOperational: operational status populated: {}",
				toaster);
	}

	private RpcError makeToasterOutOfBreadError() {
		return RpcResultBuilder.newError(ErrorType.APPLICATION,
				"resource-denied", "Toaster is out of bread", "out-of-stock",
				null, null);
	}
